...
public protocol OBWFilteringMenuDelegate {
	/// The given menu will appear.  Menu items may be added or removed at this
	///  point.
...
/// Default `OBWFilteringMenuDelegate` implementations.
...
public extension OBWFilteringMenu {
	/// Value returned by a delegate to indicate whether a menu should appear
	/// immediately or at a later time.
	enum DisplayTiming {
		/// The menu is ready to appear immediately.
		case now
		/// The menu will be ready to appear later with a call to
		/// `OBWFilteringMenu.appearNow(with:)`.
		case later
	}
}
