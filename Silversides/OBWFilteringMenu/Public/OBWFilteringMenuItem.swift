...
public class OBWFilteringMenuItem {
	...
	/// The title of the menu item, rendered in the standard menu font.  If both
	/// `title` and `attributedTitle` are present, `attributedTitle` is used.
	public var title: String?
	
	/// An attributed menu item title.  If both `title` and `attributedTitle`
	/// are present, `attributedTitle` is used.
	...
	public var submenu: OBWFilteringMenu? {
		willSet {
	...
	/// An object associated with the menu item.  The menu item holds a strong
	/// reference to the object, but does not use it in any way.
	public var representedObject: Any?
	
	/// A closure to be called when the menu item is selected.  Takes a
	/// reference to the menu item and returns nothing.
	...
	/// Adds an alternate menu item to the receiver.  An alternate cannot be
	/// added to a menu item that is itself an alternate.  The alternate menu
	/// item’s modifier mask must be different than the receiver’s mask.
	...
	/// Returns the receiver’s alternate menu item that matches the given mask.
	/// If no alternates match, then the receiver is returned.
	...
	weak var menu: OBWFilteringMenu?
	...
	/// The template image that should be used to draw the menu item’s current
	/// checked state.
